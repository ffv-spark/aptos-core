...
### 2.3.1 无序交易（Orderless Transactions）
...
**容量检查** (`mempool/src/core_mempool/transaction_store.rs:335-346`):
...
match txn_replay_protector {
    ...
    ReplayProtector::SequenceNumber(_) => {
        if txns.seq_num_txns_len() >= self.capacity_per_user {
            return MempoolStatus::new(MempoolStatusCode::TooManyTransactions).with_message(
                format!(
                    "Mempool over capacity for account. Number of sequence number transactions: {} Capacity: {}",
    ...
    ReplayProtector::Nonce(_) => {
        if txns.orderless_txns_len() >= self.orderless_txn_capacity_per_user {
            return MempoolStatus::new(MempoolStatusCode::TooManyTransactions).with_message(
                format!(
                    "Mempool over capacity for account. Number of orderless transactions: {} Capacity: {}",
...
**关键特性**：无序交易**始终处于就绪状态**，不会进入 ParkingLot！
...
**垃圾回收时的影响** (`mempool/src/core_mempool/transaction_store.rs:944-945`):

```rust
// If a sequence number transaction is garbage collected, then its subsequent transactions
// are marked as non-ready.
...
**文件**: `types/src/on_chain_config/aptos_features.rs:141-142`

```rust
pub enum FeatureFlag {
    ...
    TRANSACTION_PAYLOAD_V2 = 93,
    ORDERLESS_TRANSACTIONS = 94,  // ← 无序交易特性标志
    ...
}
```

**启用检查** (`types/src/on_chain_config/aptos_features.rs:446-448`):
...
**验证时的检查** (`aptos-move/aptos-vm/src/transaction_validation.rs:247-256`):
...
let txn_sequence_number = match txn_replay_protector {
    ...
    ReplayProtector::Nonce(_) => {
        return Err(VMStatus::error(
            ...
            Some("Orderless transactions is not supported without unified_prologue methods".to_string()),
        ));
    },
};
```
...
nonce1  nonce2  nonce3  ...  nonce10
  ↓       ↓       ↓              ↓
BlockSTM 并行执行引擎分析数据依赖:
- 如果没有冲突：全部并行执行 → 10x 加速
- 如果有冲突：仅冲突部分串行 → 仍有部分加速
...
**BlockSTM 并行执行示例**：
```
Txn1: 读 resource_A, 写 resource_B
Txn2: 读 resource_C, 写 resource_D  ← 无冲突，可并行
Txn3: 读 resource_A, 写 resource_E  ← 与 Txn1 读冲突，但可乐观并行
Txn4: 写 resource_B                 ← 与 Txn1 写冲突，必须等待

BlockSTM 执行顺序:
1. Txn1, Txn2 并行执行
2. Txn3 乐观并行执行（假设 Txn1 已完成读）
3. Txn4 等待 Txn1 完成
4. 验证 Txn3 的读是否有效，无效则重新执行

结果: 2-3 笔交易并行，而非 4 笔串行
```
...
混合模式:
- 100 笔序列号交易 + 1000 笔无序交易 = 1100 笔/用户
```

**网络整体容量提升**：
```
假设 10,000 个活跃账户:
- 传统方式: 10,000 × 100 = 1,000,000 笔
- 混合模式: 10,000 × 1,100 = 11,000,000 笔
- 提升: 11x 容量
```
...
```
用户提交:
  Txn A: seq 5 (当前账户 seq = 3)
  Txn B: seq 6
  Txn C: seq 7

如果 seq 3 或 4 的交易失败或延迟:
  → Txn A 进入 ParkingLot（未就绪）
  → Txn B 进入 ParkingLot
  → Txn C 进入 ParkingLot
  → 可能超时被清理
...
- `types/src/transaction/mod.rs:746-751` - TransactionExtraConfig（包含 nonce）
...
- `mempool/src/core_mempool/transaction_store.rs:235-240` - insert() 方法签名
