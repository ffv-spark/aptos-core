...
> **注意**：无序交易的完整实现细节已移至独立文档。

Aptos 支持**无序交易**（Orderless Transactions），这是一项重要创新，通过使用 **Nonce** 代替 **Sequence Number**，打破了传统区块链中账户交易必须严格按序执行的限制。

**核心优势**：
- **提升 TPS**: 3-10 倍性能提升（10K → 30K-100K+ TPS）
- **更高并发**: 交易可以并行执行，无需等待前序交易
- **更大容量**: 每用户 1000 笔无序交易 vs 100 笔序列号交易
- **始终就绪**: 无序交易不会进入 ParkingLot，立即可执行

**Mempool 配置**：
```rust
pub struct MempoolConfig {
    pub capacity_per_user: usize,                    // 100 笔序列号交易
    pub orderless_txn_capacity_per_user: usize,      // 1000 笔无序交易
}
```

**📖 完整文档**: 请查看 **[docs/orderless-transactions.md](./orderless-transactions.md)** 获取：
- 交易格式和数据结构
- 链上 Nonce 管理机制（NonceHistory、50K 哈希桶、SipHash）
- VM Prologue 验证流程
- Mempool 处理逻辑
- 完整交易生命周期（7 阶段 31 步）
- 重放攻击防护机制
- 性能优化技巧
- Rust 和 TypeScript 使用示例
