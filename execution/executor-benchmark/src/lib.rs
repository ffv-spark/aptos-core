...
fn init_indexer_wrapper(
    ...
    start_version: u64,
) -> Option<(Arc<IndexerAsyncV2>, Arc<AtomicU64>)> {
    ...
    let indexer_db = open_db(db_path, &rocksdb_config).expect("Failed to open indexer async v2 db");
    ...
    indexer_runtime.spawn(async move {
        let grpc_service = FullnodeDataService { service_context };
    ...
    Some((indexer_async_v2, grpc_version))
}
...
fn create_checkpoint(
    ...
    if enable_indexer_grpc {
        ...
        let indexer_db =
            open_db(db_path, &Default::default()).expect("Failed to open table info db.");
        indexer_db
